...
for f in $(cat $DIST_ROOT/build_env/build_list)
do
    bn=$(basename $f)
...
if ! test $(id -u tester) ; then
groupadd tester
useradd -s /bin/bash -g tester -m -k /dev/null tester
passwd distbuild
chown -v tester $LFS/{usr,lib,var,etc,bin,sbin,tools,lib64,sources}
echo "tester ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/sudoers_tester
dbhome=$(eval echo "~tester")
